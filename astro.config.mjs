...
export default defineConfig({
  // 1. БАЗОВЫЕ НАСТРОЙКИ (Твой новый конфиг)
  ...
  integrations: [
    ...
    mdx(),
    sitemap({
      filter: (page) => !page.includes('/links/') && !page.includes('/404')
    })
  ],
  ...
  // 2. БЛОК РЕДИРЕКТОВ (Критически важно для Google Search Console)
  // Без этого блока твои ошибки индексации не исправятся
  redirects: {
    // Служебные страницы
    ...
    // Исправление структуры товаров (старые ссылки -> новые)
    ...
    // ЧИСТКА РАЗДЕЛА ВИДЕО (Массовый редирект на товары)
    ...
    // Перенаправляем всё остальное из /video на главную рекомендаций
    '/video/[...slug]': { status: 301, destination: '/empfehlungen/' },

    // Убираем мусорную ссылку поиска
